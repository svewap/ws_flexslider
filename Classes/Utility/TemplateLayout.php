...
class TemplateLayout implements SingletonInterface
{
    ...
    public function getAvailableTemplateLayouts($pageUid)
    {
        ...
        if (isset($GLOBALS['TYPO3_CONF_VARS']['EXT']['ws_flexslider']['templateLayouts'])
            && is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['ws_flexslider']['templateLayouts'])
        ) {
            $templateLayouts = $GLOBALS['TYPO3_CONF_VARS']['EXT']['ws_flexslider']['templateLayouts'];
        }
    ...
    protected function getTemplateLayoutsFromTsConfig($pageUid)
    {
        ...
        if (isset($pagesTsConfig['tx_wsflexslider.']['templateLayouts.']) && is_array($pagesTsConfig['tx_wsflexslider.']['templateLayouts.'])) {
            $templateLayouts = $pagesTsConfig['tx_wsflexslider.']['templateLayouts.'];
        }
        return $templateLayouts;
    }
}